...
export default function RecognitionPage() {
  ...
  React.useEffect(() => {
    let mounted = true
      ; (async () => {
        try {
          // Fetch from the new local MongoDB profile endpoint
          if (user && mounted) {
            const res = await fetch('/api/user/profile');
            if (res.ok) {
              const data = await res.json();
              if (typeof data.prefer_gemini === 'boolean') {
  ...
  // Persist user-upload + gemini result to local MongoDB for dataset building
  const storeAnnotation = async (file: File, gemini: any) => {
    try {
      ...
      formData.append('hash', hex);
      if (gemini) {
        formData.append('gemini', JSON.stringify(gemini));
      }
      ...
      if (!res.ok) {
        console.warn('Failed to insert annotation', await res.text());
      } else {
        ...
        <main className="container py-10 flex flex-col items-center justify-center">
          ...
          <CommunityPostModal
            ...
            onPost={async (description) => {
              try {
                // user object is already available in the component via useAuth()
